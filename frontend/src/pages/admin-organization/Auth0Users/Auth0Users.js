...
import Auth0UserTable from "src/components/dashboard/organization/Auth0UserTable/Auth0UserTable";
...
export default function Auth0Users() {
  ...
  const fetchData = async () => {
    try {
      const resp = await apiClients.getAllAuth0users();
      if (resp) {
        setUsers(resp);
        setLoader(false);
      }
        ...
        <Card>
          <Box sx={{ width: "100%", typography: "body1", padding: "10px" }}>
            {loader ? (
              ...
              <Auth0UserTable userData={users} fetchData={fetchData} />
            )}
          </Box>
        </Card>
      </Container>
